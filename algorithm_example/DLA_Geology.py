...
def loss_and_opt(net):
    ...
    exp_lr = torch.optim.lr_scheduler.ExponentialLR(opt, gamma=0.99)
...
class DLA(nn.Module):
    def __init__(self, block=BasicBlock, num_classes=label_num):
        self.model_type = "general"  # general 或者 seq2seq
        super(DLA, self).__init__()
        self.base = nn.Sequential(
            nn.Conv1d(features_num, 16, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm1d(16),
            nn.ReLU(True)
        )
