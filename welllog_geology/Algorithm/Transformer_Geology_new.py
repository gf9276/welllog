...
class NN(nn.Module):
    def __init__(self, input_size, magnification):
        super(NN, self).__init__()
        # self.fc1构建第一个全连接层（full connect），nn.Linear是torch的全连接层方法，两个参数是左右的神经元个数
        hidden1_size = input_size * magnification
        hidden2_size = input_size * magnification
        output_size = input_size * magnification
        self.magnification = magnification
        self.layer1 = nn.Sequential(nn.Linear(input_size, hidden1_size), nn.BatchNorm1d(hidden1_size), nn.ReLU(True))
        self.layer2 = nn.Sequential(nn.Linear(hidden1_size, hidden2_size), nn.BatchNorm1d(hidden2_size), nn.ReLU(True))
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        return x.reshape(x.size()[0], x.size()[1] // self.magnification, self.magnification)


class FeatureEncoder(nn.Module):
    def __init__(self, nn_nbr, input_size, magnification):
        super(FeatureEncoder, self).__init__()
        self.nns = nn.ModuleList([NN(input_size, magnification) for i in range(nn_nbr)])

    def forward(self, x):
        output = []
        for i in range(len(self.nns)):
            output.append(self.nns[i](x[:, :, i]))
...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, feature_encoder, encoder, decoder, src_embed, tgt_embed, generator):
        """
        生成编码解码器
        :param encoder: 是个函数
        :param decoder: 是个函数
    ...
    def forward(self, src, src_mask, tgt=None, tgt_mask=None, just_encoder=True):
        ...
        if just_encoder:
            return self.generator(self.encode(self.feature_encoder(src), src_mask))
...
def make_model(h_in, tgt_vocab, n=6, d_model=512, d_ff=2048, h=8, dropout=0.1, nn_input_size=slicelength, magnification=3):
    ...
    model = EncoderDecoder(
        FeatureEncoder(h_in, nn_input_size, magnification),
        Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout), n),
        Decoder(DecoderLayer(d_model, c(attn), c(attn), c(ff), dropout), n),
        nn.Sequential(EmbeddingsLiner(h_in * magnification, d_model), c(position)),  # 这里被我改成了维度转换
